...
Psim(t,:)=Pc;
t=t+1;
waitbar(t/length(rc),hwait);
end
close(hwait);
plot(thresh_dB,Psim(1,:),'b-')
grid on
hold on
plot(thresh_dB,Psim(2,:),'k-o')
plot(thresh_dB,Psim(3,:),'m->')
plot(thresh_dB,Psim(4,:),'r-d')
legend('Sim: rc = 50m','Sim: rc = 100m','Sim: rc = 200m','Sim: rc = 300m')
